...
export class ViewState implements IViewState {
   ...
   */
  constructor(centerX: number = -0.5, centerY: number = 0.0, zoom: number = 1.0) {
  ...
  toScreenCoords(fractalX: number, fractalY: number, screenWidth: number, screenHeight: number): [number, number] {
    const screenX = (fractalX - this.centerX) * this.zoom * screenWidth + screenWidth * 0.5;
    const screenY = -(fractalY - this.centerY) * this.zoom * screenHeight + screenHeight * 0.5;
